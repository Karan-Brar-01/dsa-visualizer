// ─────────────────────────────────────────────────────────────────────────────
// src/stores/bstStore.ts
//
// Zustand store for the Binary Search Tree visualizer.
// ─────────────────────────────────────────────────────────────────────────────
...
import { BinarySearchTree } from '@/core/trees/bst/BinarySearchTree'
import type { BSTSnapshot } from '@/core/trees/bst/types'
...
interface MathematicalState {
  readonly _engine: BinarySearchTree
...
interface AnimationState {
  ...
  pendingResult: OperationResult<BSTSnapshot> | null
  operationLog: string[]
...
interface InternalActions {
  ...
  _dispatchOperation: (result: OperationResult<BSTSnapshot>, logMessage: string) => void
...
function computeLayout(snapshot: BSTSnapshot): {
...
const initialEngine = new BinarySearchTree()
const initialSnapshot = initialEngine.reset()

export const useBSTStore = create<BSTStore>()(
  immer((set, get) => ({
    // MathematicalState
      ...
      set((state) => {
        ...
        if (step.pointerMutation) {
          ...
          if (newTargetId) {
           ...
           // In a tree, every node (except root) has exactly 1 incoming edge.
    ...
    _syncVisualState: () => {
      ...
      const isDelete = pendingResult?.kind === 'BST_DELETE'
      ...
      set((state) => {
        ...
        if (pendingResult && pendingResult.success) {
            if (pendingResult.kind === 'BST_INSERT') {
...
export const selectors = {
  rfNodes: (s: BSTStore) => s.rfNodes,
  rfEdges: (s: BSTStore) => s.rfEdges,
  isPlaying: (s: BSTStore) => s.isPlaying,
  currentStepIndex: (s: BSTStore) => s.currentStepIndex,
  playbackSpeed: (s: BSTStore) => s.playbackSpeed,
  snapshot: (s: BSTStore) => s.snapshot,
  operationLog: (s: BSTStore) => s.operationLog,
  currentStep: (s: BSTStore) => s.currentStep,
  listSize: (s: BSTStore) => s.listSize,
  actions: (s: BSTStore) => ({
    insert: s.insert,
    search: s.search,
    delete: s.delete,
