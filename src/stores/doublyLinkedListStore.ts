// ─────────────────────────────────────────────────────────────────────────────
// src/stores/doublyLinkedListStore.ts
//
...
//     → Core engine (DoublyLinkedList) returns OperationResult
...
import { DoublyLinkedList } from '@/core/linked-list/DoublyLinkedList'
import type { DoublyListSnapshot } from '@/core/linked-list/types'
...
interface MathematicalState {
  ...
  readonly _engine: DoublyLinkedList
  /** Live snapshot of the engine state. Kept in sync with _engine. */
  snapshot: DoublyListSnapshot
}
...
interface AnimationState {
  ...
  pendingResult: OperationResult<DoublyListSnapshot> | null
...
interface InternalActions {
   ...
   */
  _dispatchOperation: (result: OperationResult<DoublyListSnapshot>) => void
 ...
 */
function computeLayout(snapshot: DoublyListSnapshot): {
  ...
  orderedIds.forEach((id, index) => {
    ...
    if (node.next !== null) {
      edges.push({
        ...
        targetHandle: 'left',
        type: 'pointerEdge',
        data: { isTraversing: false, isNull: false },
        animated: false,
      })
    } else {
      // Terminal edge → NULL sentinel
      const nullId = `null-sentinel-${id}-next`
      nodes.push({
        id: nullId,
        type: 'nullSentinel',
        position: {
          x: index * NODE_HORIZONTAL_SPACING + NULL_SENTINEL_OFFSET,
          y: NODE_Y,
        },
        data: {} as ListNodeData,
        draggable: false,
        selectable: false,
      })
      edges.push({
        ...
        targetHandle: 'left',
        type: 'pointerEdge',
        data: { isTraversing: false, isNull: true },
        animated: false,
      })
    }
    ...
    if (node.prev !== null) {
      edges.push({
        id: `edge-${id}-prev-${node.prev}`,
        source: id,
        target: node.prev,
        sourceHandle: 'top', // use top handles for prev curve
        targetHandle: 'top',
        type: 'pointerEdge',
        data: { isTraversing: false, isNull: false, isPrev: true },
        animated: false,
      })
    } else {
      // Terminal edge → NULL sentinel for prev
      const nullId = `null-sentinel-${id}-prev`
      nodes.push({
        id: nullId,
        type: 'nullSentinel',
        position: {
      ...
      edges.push({
        id: `edge-${id}-prev-null`,
        source: id,
        target: nullId,
        sourceHandle: 'left',
        targetHandle: 'right',
        type: 'pointerEdge',
        data: { isTraversing: false, isNull: true, isPrev: true },
        animated: false,
      })
    }
  })
...
const engine = new DoublyLinkedList()

const initialSnapshot: DoublyListSnapshot = engine.getSnapshot()

const INITIAL_STATE: Omit<
  DoublyLinkedListStore,
...
export const useDoublyLinkedListStore = create<DoublyLinkedListStore>()(
  immer((set, get) => ({
    ...INITIAL_STATE,
    ...
    applyStep(step) {
      set((state) => {
        ...
        if (step.pointerMutation) {
          const { sourceId, newTargetId, pointerType = 'next' } = step.pointerMutation
          state.rfEdges = state.rfEdges.map((e) => {
            const isPrevEdge = e.data?.isPrev ?? false
            const isTargetPrev = pointerType === 'prev'
            
            const isMatch = e.source === sourceId && 
                            e.target === newTargetId && 
                            isPrevEdge === isTargetPrev

            return {
              ...e,
              data: {
                ...e.data,
                isTraversing: isMatch,
              } as PointerEdgeData,
              animated: isMatch,
            }
          })
 ...
 * Example: const rfNodes = useDoublyLinkedListStore(selectors.rfNodes)
 */
export const selectors = {
  rfNodes: (s: DoublyLinkedListStore) => s.rfNodes,
  rfEdges: (s: DoublyLinkedListStore) => s.rfEdges,
  snapshot: (s: DoublyLinkedListStore) => s.snapshot,
  isPlaying: (s: DoublyLinkedListStore) => s.isPlaying,
  currentStep: (s: DoublyLinkedListStore) => s.steps[s.currentStepIndex] ?? null,
  steps: (s: DoublyLinkedListStore) => s.steps,
  currentStepIndex: (s: DoublyLinkedListStore) => s.currentStepIndex,
  playbackSpeed: (s: DoublyLinkedListStore) => s.playbackSpeed,
  operationLog: (s: DoublyLinkedListStore) => s.operationLog,
  listSize: (s: DoublyLinkedListStore) => s.snapshot.size,
  actions: (s: DoublyLinkedListStore): MathActions & AnimationActions => ({
