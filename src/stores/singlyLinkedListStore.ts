// ─────────────────────────────────────────────────────────────────────────────
// src/stores/singlyLinkedListStore.ts
//
...
//     → Core engine (SinglyLinkedList) returns OperationResult
...
import { SinglyLinkedList } from '@/core/linked-list/SinglyLinkedList'
...
interface MathematicalState {
  ...
  readonly _engine: SinglyLinkedList
...
export const useSinglyLinkedListStore = create<SinglyLinkedListStore>()(
  immer((set, get) => ({
    ...INITIAL_STATE,
    ...
    insertAt(index, value) {
      const result = get()._engine.insertAt(index, value)
      get()._dispatchOperation(result)
    },
    ...
    deleteAt(index) {
      const result = get()._engine.deleteAt(index)
      get()._dispatchOperation(result)
    },
 ...
 * Example: const rfNodes = useSinglyLinkedListStore(selectors.rfNodes)
 */
export const selectors = {
  rfNodes: (s: SinglyLinkedListStore) => s.rfNodes,
  rfEdges: (s: SinglyLinkedListStore) => s.rfEdges,
  snapshot: (s: SinglyLinkedListStore) => s.snapshot,
  isPlaying: (s: SinglyLinkedListStore) => s.isPlaying,
  currentStep: (s: SinglyLinkedListStore) => s.steps[s.currentStepIndex] ?? null,
  steps: (s: SinglyLinkedListStore) => s.steps,
  currentStepIndex: (s: SinglyLinkedListStore) => s.currentStepIndex,
  playbackSpeed: (s: SinglyLinkedListStore) => s.playbackSpeed,
  operationLog: (s: SinglyLinkedListStore) => s.operationLog,
  listSize: (s: SinglyLinkedListStore) => s.snapshot.size,
  actions: (s: SinglyLinkedListStore): MathActions & AnimationActions => ({
