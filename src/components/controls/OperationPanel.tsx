...
const OPERATIONS: OpDef[] = [
  ...
  { mode: 'insertTail', label: 'Insert Tail', shortLabel: 'Tail', needsValue: true,  needsIndex: false, group: 'insert', complexity: 'O(n)', accent: 'violet' },
  { mode: 'insertAt',   label: 'Insert At',   shortLabel: 'At',   needsValue: true,  needsIndex: true,  group: 'insert', complexity: 'O(n)', accent: 'violet' },
  { mode: 'deleteHead', label: 'Delete Head', shortLabel: 'Head', needsValue: false, needsIndex: false, group: 'delete', complexity: 'O(1)', accent: 'red' },
  ...
  { mode: 'deleteAt',   label: 'Delete At',   shortLabel: 'At',   needsValue: false, needsIndex: true,  group: 'delete', complexity: 'O(n)', accent: 'red' },
  { mode: 'search',     label: 'Search',      shortLabel: 'Val',  needsValue: true,  needsIndex: false, group: 'search', complexity: 'O(n)', accent: 'cyan' },
]
...
const GROUP_LABELS = {
  ...
  search: { label: 'Search', color: 'text-cyan-400',   active: 'bg-cyan-500/20 border-cyan-500/40 text-cyan-300',       inactive: 'text-[hsl(210,8%,50%)] hover:text-[hsl(210,12%,70%)]' },
}
...
export function OperationPanel() {
  ...
  const actions   = useSinglyLinkedListStore(selectors.actions)
  const isPlaying = useSinglyLinkedListStore(selectors.isPlaying)
  const hasSteps  = useSinglyLinkedListStore((s) => s.steps.length > 0)
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    switch (mode) {
      ...
      case 'insertAt':   actions.insertAt(numIndex, numValue); break
      case 'deleteHead': actions.deleteHead(); break
      case 'deleteTail': actions.deleteTail(); break
      case 'deleteAt':   actions.deleteAt(numIndex); break
