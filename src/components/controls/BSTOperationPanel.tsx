...
export function BSTOperationPanel() {
  const id = useId()
  const [mode, setMode] = useState<OperationMode>('insert')
  const [value, setValue] = useState('')

  const actions   = useBSTStore(selectors.actions)
  const isPlaying = useBSTStore(selectors.isPlaying)
  const hasSteps  = useBSTStore((s) => s.steps.length > 0)
  ...
  return (
    <form
      ...
      aria-label="BST operation controls"
    >
