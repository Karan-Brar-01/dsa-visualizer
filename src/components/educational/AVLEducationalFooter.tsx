...
const COMPLEXITY_TABLE = [
  ...
  { op: 'Worst Case',  time: 'O(log n)', space: 'O(1)', accent: 'text-amber-400' },
]
...
export function AVLEducationalFooter() {
  const operationLog     = useAVLStore(selectors.operationLog)
  const currentStep      = useAVLStore(selectors.currentStep)
  const listSize         = useAVLStore(selectors.listSize)
  const snapshot         = useAVLStore(selectors.snapshot)
  const isPlaying        = useAVLStore(selectors.isPlaying)
  const currentStepIndex = useAVLStore(selectors.currentStepIndex)
  const totalSteps       = useAVLStore((s) => s.steps.length)
